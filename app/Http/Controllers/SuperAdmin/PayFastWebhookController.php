...
class PayFastWebhookController extends Controller
{
    public function saveInvoice(Request $request)
    {
        ...
        if ($check1 && $check2 && $check4) {
            ...
            GlobalSubscription::where('society_id', $subscription->society_id)->where('subscription_status', 'active')->update(['subscription_status' => 'inactive', 'ends_at' => now()]);
            ...
            $invoice->society_id = $request->custom_int1;
            ...
            $society_id = Society::find($subscription->society_id);
            $society_id->package_id = $subscription->package_id;
            $society_id->package_type = $subscription->package_type;

            // Set society_id status active
            $society_id->status = 'active';
            $society_id->license_expire_on = null;
            $society_id->save();
        }
    }
