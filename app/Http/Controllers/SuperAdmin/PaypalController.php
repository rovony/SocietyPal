...
use App\Models\{SuperadminPaymentGateway, SocietyPayment, Society, Package, GlobalSubscription, User, GlobalInvoice};
...
class PaypalController extends Controller
{
    ...
    public function initiatePayment(Request $request)
    {
        ...
        return $package->package_type->value === 'lifetime'
            ? $this->handleLifetimePayment($societyPayment, $package)
            : $this->handleSubscriptionPayment($request, $societyPayment, $package);
    }

    private function handleLifetimePayment($societyPayment, $package)
    {
        ...
        $paypalData = [
            ...
            "purchase_units" => [[
                ...
                "reference_id" => (string) $societyPayment->id
            ]],
            "application_context" => [
                "return_url" => url('/paypal/lifetime/success?payment_id=' . $societyPayment->id),  // Ensure full URL here
    ...
    public function paypalLifetimeSuccess(Request $request)
    {
        ...
        if (!$response->successful()) {
            ...
            $societyPayment->update(['status' => 'failed']);
        ...
        $society = Society::findOrFail($societyPayment->society_id);
        $package = Package::findOrFail($societyPayment->package_id);
    
        // Update society details based on the successful payment
        $society->update([
    ...
    private function handleSubscriptionPayment(Request $request, $societyPayment, $package)
    {
        ...
        $society = Society::findOrFail(society()->id);
        ...
        $societyPayment->paypal_payment_id = $newPlan->getId();
        $societyPayment->save();
        ...
        $paypalSubscription->society_id = society()->id;
        ...
        $paypalInvoice->society_id = society()->id;
        ...
        if($requestObject->get('success') == true && $requestObject->has('token') && $requestObject->get('success') != 'false' )
        {
            ...
            try {
                ...
                if($agreement->getState() == 'Active' || $agreement->getState() == 'Pending') {
                    ...
                    $society->package_id = $clientPayment->package_id;
                    $society->package_type = ($clientPayment->billing_frequency == 'year') ? 'annual' : 'monthly';
                    $society->status = 'active';
                    $society->license_expire_on = null;
                    $society->save();

                    if( $society->package_type == 'monthly') {
                    ...
                    Notification::send($generatedBy, new SocietyUpdatedPlan($society, $clientPayment->package_id));
        
                    // Notify society admin
                    $societyAdmin = $society->societyAdmin($society);
                    Notification::send($societyAdmin, new SocietyUpdatedPlan($society, $clientPayment->package_id));
    ...
    public function verifyBillingIPN(Request $request)
    {
        ...
        if ($txnType == 'recurring_payment') {
            ...
            if($event == 0)
            {
                ...
                $paypalInvoice->society_id = $payment->society_id;
                ...
                $paypalInvoice->save();
                $society = Society::findOrFail($payment->society_id);
                $society->status = 'active';
                $society->save();
                $generatedBy = User::whereNull('society_id')->get();

                Notification::send($generatedBy, new SocietyUpdatedPlan($society, $payment->plan_id));
