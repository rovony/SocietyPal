...
class UpdatePackage extends Component
{
    ...
    {
        $this->latestSubscription = $this->society;
        $this->packages = Package::all();
        $this->packageId = $this->society->package_id;
    ...
    public function updatePackageSubmit()
    {
        ...
        $society = Society::with('package')->findOrFail($this->society->id);
        $package = Package::findOrFail($this->selectedPackage->id);
        $isTrial = $this->packageType === PackageType::TRIAL;
        $billingCycle = $this->billingCycle;

        try {
            // Update the society with the new package details
            $society->package_id = $package->id;
            $society->package_type = $billingCycle;
            $society->trial_ends_at = $isTrial ? \Carbon\Carbon::parse($this->trialExpireOn)->format('Y-m-d') : null;
            ...
            $licenceExpireOn = $this->licenceExpireOn ? \Carbon\Carbon::parse($this->licenceExpireOn) : null;
            $society->license_expire_on = match ($billingCycle) {
    ...
    public function validatePackageData()
    {
        ...
        if ($this->packageType === PackageType::TRIAL) {
            ...
            if ($this->packageType != PackageType::LIFETIME) {
