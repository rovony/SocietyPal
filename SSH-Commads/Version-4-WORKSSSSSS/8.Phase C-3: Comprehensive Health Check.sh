...
if [ -f "artisan" ]; then
    if php artisan --version >/dev/null 2>&1; then
        ...
        if apply_fix "Reinstall Laravel Dependencies" "$COMPOSER_CMD install --no-dev --optimize-autoloader --no-interaction" "Fix missing or corrupted Laravel dependencies"; then
