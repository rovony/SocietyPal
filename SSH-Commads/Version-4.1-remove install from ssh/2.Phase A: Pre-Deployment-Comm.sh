...
# Check for Composer (informational only - dependencies handled by build process)
if command -v composer2 &> /dev/null; then
    composer2 --version
    echo "✅ Composer 2 available (dependencies pre-built)"
elif composer --version | grep -q "version 2\."; then
    composer --version
    echo "✅ Composer 2 detected (dependencies pre-built)"
else
    composer --version 2>/dev/null || echo "ℹ️ Composer not required (dependencies pre-built)"
    echo "ℹ️ Dependencies handled by build process - no server-side installation needed"
fi
