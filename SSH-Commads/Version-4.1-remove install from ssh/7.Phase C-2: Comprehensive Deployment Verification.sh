...
if [ -n "$CURRENT_RELEASE" ] && [ "$CURRENT_RELEASE" != "NONE" ] && [ -d "$CURRENT_PATH" ]; then
    ...
    if [ -f "artisan" ]; then
        ...
        if php artisan tinker --execute="try { DB::connection()->getPdo(); echo 'OK'; } catch(Exception \$e) { echo 'FAILED'; }" 2>/dev/null | grep -q "OK"; then
            ...
            # ENHANCED: Test migration status and schema integrity
            echo "🔍 Testing database schema integrity..."
            
            # Check if migrations table exists and has recent entries
            MIGRATION_CHECK=$(php artisan tinker --execute="
                try {
                    \$migrations = DB::table('migrations')->count();
                    \$recent = DB::table('migrations')->where('batch', '>', 0)->count();
                    echo \"MIGRATIONS_OK:\$migrations:\$recent\";
                } catch(Exception \$e) {
                    echo 'MIGRATIONS_FAILED: ' . \$e->getMessage();
                }
            " 2>/dev/null)
            
            if echo "$MIGRATION_CHECK" | grep -q "MIGRATIONS_OK"; then
                MIGRATION_COUNT=$(echo "$MIGRATION_CHECK" | cut -d':' -f2)
                RECENT_COUNT=$(echo "$MIGRATION_CHECK" | cut -d':' -f3)
                MIGRATION_STATUS="✅ HEALTHY ($MIGRATION_COUNT migrations, $RECENT_COUNT batched)"
                echo "📋 Migrations: $MIGRATION_STATUS"
                
                # Test critical Laravel tables exist
                SCHEMA_CHECK=$(php artisan tinker --execute="
                    try {
                        \$tables = ['users', 'password_resets', 'failed_jobs'];
                        \$existing = [];
                        foreach(\$tables as \$table) {
                            if(Schema::hasTable(\$table)) \$existing[] = \$table;
                        }
                        echo 'SCHEMA_OK:' . count(\$existing) . '/' . count(\$tables);
                    } catch(Exception \$e) {
                        echo 'SCHEMA_FAILED: ' . \$e->getMessage();
                    }
                " 2>/dev/null)
                
                if echo "$SCHEMA_CHECK" | grep -q "SCHEMA_OK"; then
                    SCHEMA_RATIO=$(echo "$SCHEMA_CHECK" | cut -d':' -f2)
                    SCHEMA_STATUS="✅ CORE TABLES ($SCHEMA_RATIO)"
                    echo "🏗️ Schema: $SCHEMA_STATUS"
                else
                    SCHEMA_STATUS="❌ INCOMPLETE"
                    echo "🏗️ Schema: $SCHEMA_STATUS - Core tables missing or inaccessible"
                fi
            else
                MIGRATION_STATUS="❌ FAILED"
                SCHEMA_STATUS="❌ UNKNOWN"
                echo "📋 Migrations: $MIGRATION_STATUS - Cannot access migrations table"
                echo "🏗️ Schema: $SCHEMA_STATUS - Cannot verify due to migration issues"
            fi
        else
            DB_STATUS="❌ FAILED"
            MIGRATION_STATUS="❌ NO_CONNECTION"
            SCHEMA_STATUS="❌ NO_CONNECTION"
            echo "🗄️ Database: Connection Failed"
            echo "📋 Migrations: Cannot test (no database connection)"
            echo "🏗️ Schema: Cannot test (no database connection)"
        fi
...
- **Migrations**: $([ "$MIGRATION_STATUS" = "✅ HEALTHY"* ] && echo "✅ Schema up-to-date" || echo "❌ Migration issues detected")
- **Schema Integrity**: $([ "$SCHEMA_STATUS" = "✅ CORE TABLES"* ] && echo "✅ Core tables verified" || echo "❌ Schema incomplete or missing")
...
[[ "$MIGRATION_STATUS" =~ "✅ HEALTHY" ]] && PASSED_CHECKS=$((PASSED_CHECKS + 1))
[[ "$SCHEMA_STATUS" =~ "✅ CORE TABLES" ]] && PASSED_CHECKS=$((PASSED_CHECKS + 1))
...
- **Database Migrations:** $([[ "$MIGRATION_STATUS" =~ "✅ HEALTHY" ]] && echo "✅ Schema Current" || echo "⚠️ Migration Issues")
- **Schema Integrity:** $([[ "$SCHEMA_STATUS" =~ "✅ CORE TABLES" ]] && echo "✅ Tables Verified" || echo "⚠️ Schema Problems")
...
if [[ ! "$MIGRATION_STATUS" =~ "✅ HEALTHY" ]]; then
    echo "- ? **Fix migration issues** - Database schema may be incomplete or corrupted" >> "$REPORT_FILE"
    echo "   - Run: \`php artisan migrate --force\` to update schema" >> "$REPORT_FILE"
    echo "   - Check: \`php artisan migrate:status\` for pending migrations" >> "$REPORT_FILE"
fi

if [[ ! "$SCHEMA_STATUS" =~ "✅ CORE TABLES" ]]; then
    echo "- ? **Verify database schema** - Core Laravel tables missing or inaccessible" >> "$REPORT_FILE"
    echo "   - This often indicates failed migrations or Faker dependency issues" >> "$REPORT_FILE"
    echo "   - Check migration logs and ensure build artifacts contain all dependencies" >> "$REPORT_FILE"
fi
