...
if command -v composer2 &> /dev/null; then
    ...
    COMPOSER_RECOMMENDATION="Use existing: \`composer2 install --optimize-autoloader\`  # Dev dependencies included as needed"
    ...
    COMPOSER_RECOMMENDATION="Use existing: \`composer install --optimize-autoloader\`  # Dev dependencies included as needed"
    ...
    COMPOSER_RECOMMENDATION="**Upgrade needed:** Install Composer 2 per-domain (required for proper dependency resolution)"
    PHASE_A_ISSUES=$((PHASE_A_ISSUES + 1))
