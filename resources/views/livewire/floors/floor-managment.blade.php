<div class="px-4 pt-2 bg-white xl:grid-cols-3 xl:gap-4 dark:bg-gray-900">
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700" wire:key='member-list-{{ microtime() }}'>
                            ...
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700" wire:key='member-{{ $item->id . rand(1111, 9999) . microtime() }}' wire:loading.class.delay='opacity-10'>
                                ...
                                <td class="p-4 text-base font-semibold text-gray-900 whitespace-nowrap dark:text-white">
                                    <a class="text-base font-semibold hover:underline dark:text-black-400" href="{{ route('floors.show' , $item->id)}}">{{ $item->floor_name }}</a>
                                </td>
                                ...
                                <td class="p-4 space-x-2 text-right whitespace-nowrap">
                                    @if(user_can('Update Floor'))
                                        <x-secondary-button wire:click='showEditFloor({{ $item->id }})' wire:key='member-edit-{{ $item->id . microtime() }}'
                                    ...
                                    @if(user_can('Delete Floor'))
                                        <x-danger-button  wire:click="showDeleteFloor({{ $item->id }})"  wire:key='member-del-{{ $item->id . microtime() }}'>
                            ...
                            @empty
                                <x-no-results :message="__('messages.noFloorFound')" />
                            @endforelse
    ...
    <div wire:key='floor-table-paginate-{{ microtime() }}'
        class="sticky bottom-0 right-0 items-center w-full p-4 bg-white border-t border-gray-200 sm:flex sm:justify-between dark:bg-gray-800 dark:border-gray-700">
        <div class="mb-4 sm:mb-0 w-full">
            {{ $floorData->links() }}
        </div>
    </div>
    <x-right-modal wire:model.live="showEditFloorModal">
        <x-slot name="title">
            {{ __("modules.settings.editSocietyFloor") }}
        </x-slot>

        <x-slot name="content">
            @if ($floor)
            @livewire('forms.editFloor', ['floor' => $floor], key(str()->random(50)))
            @endif
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$set('showEditFloorModal', false)" wire:loading.attr="disabled">
                {{ __('Close') }}
            </x-secondary-button>
        </x-slot>
    </x-right-modal>

    <x-confirmation-modal wire:model="confirmDeleteFloorModal">
        <x-slot name="title">
            @lang('modules.settings.deleteFloor')
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="flex items-center p-4 mt-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800" role="alert">
                ...
                <div>
                  <span class="font-medium"><strong>@lang('modules.settings.cautionMessageForFloor')</strong></span>
                </div>
            </div>
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmDeleteFloorModal')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>

            @if ($floor)
            <x-danger-button class="ml-3" wire:click='deleteFloor({{ $floor->id }})' wire:loading.attr="disabled">
                {{ __('Delete') }}
            </x-danger-button>
            @endif
         </x-slot>
    ...
    <x-confirmation-modal wire:model="confirmSelectedDeleteFloorModal">
        <x-slot name="title">
            @lang('modules.settings.deleteFloor')
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="flex items-center p-4 mt-4 mb-4 text-sm text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800" role="alert">
                ...
                <div>
                  <span class="font-medium"><strong>@lang('modules.settings.cautionMessageSelectedForFloor')</strong></span>
                </div>
            </div>
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmSelectedDeleteFloorModal')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>
