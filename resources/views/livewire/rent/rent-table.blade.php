<div class="px-4 pt-2 xl:grid-cols-3 xl:gap-4 bg-white dark:bg-gray-900">
                                ...
                                <td class="py-2.5 px-4 space-x-2 whitespace-nowrap text-right">
                                    <x-dropdown align="right">
                                        ...
                                        <x-slot name="content">
                                            ...
                                            <x-dropdown-link wire:click="showRentDetail({{ $item->id }})" class="text-yellow-500 dark:text-yellow-400">
                                                @lang('app.view')
                                            </x-dropdown-link>

                                            @if(user_can('Update Rent') && $item->status == "unpaid")
                                                <x-dropdown-link wire:click="showEditRent({{ $item->id }})" wire:key='member-edit-{{ $item->id . microtime() }}'>
                                                    @lang('app.update')
                                                </x-dropdown-link>
                                            @endif
                                            ...
                                            @if($item->status == "unpaid")
                                                @if(user_can('Delete Rent'))
                                                    <x-dropdown-link wire:click="showDeleteRent({{ $item->id }})" class="text-red-500 dark:text-red-400">
                                                        @lang('app.delete')
                                                    </x-dropdown-link>
                                                @endif
    ...
    <x-dialog-modal wire:model.live="showRentDetailModal">
        <x-slot name="title">
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$set('showRentDetailModal', false)" wire:loading.attr="disabled">
                {{ __('Close') }}
            </x-secondary-button>
        </x-slot>
    </x-dialog-modal>
