<div>
    ...
    <div class="text-xl font-bold mb-4 text-gray-700 dark:text-gray-200">@lang('modules.package.societyCurrentPackage')</div>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div class="font-semibold text-gray-700 dark:text-gray-300">{{ __('modules.package.packageDetails') }}</div>
        <div>{{ $latestSubscription->package->package_name}}</div>

        <div class="font-semibold text-gray-700 dark:text-gray-300">{{ __('modules.package.packageType') }}</div>
        <div> {{ ucfirst($latestSubscription->package_type) }}</div>
        ...
        <div>{{ $latestSubscription->currentInvoice->next_pay_date ? \Carbon\Carbon::parse($latestSubscription->currentInvoice->next_pay_date)->format('d-m-Y') : '--' }} </div>
    ...
    <form wire:submit="updatePackageSubmit" class="space-y-4">
        ...
        @if($packageType === App\Enums\PackageType::TRIAL)
            <div>
                ...
                <x-datepicker id="trial_expire_on" class="w-full mt-1" wire:model.live="trialExpireOn" autocomplete="off" placeholder="{{ __('modules.package.selectDate') }}" />
        ...
        <div class="flex w-full pb-4 space-x-4 mt-6">
            ...
            <x-button-cancel  wire:click="$dispatch('hideChangePackage')" wire:loading.attr="disabled">@lang('app.cancel')</x-button-cancel>
        </div>
    </form>
