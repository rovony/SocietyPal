    ...
    <form wire:submit.prevent="submitForm">
        ...
        <div class="flex space-x-4">
            ...
            <div>
                ...
                <x-input id="location" class="block w-full mt-1" type="text" wire:model='location'/>
                <x-input-error for="location" class="mt-2" />
            </div>

            <div>
                <x-label for="purchaseDate" value="{{ __('modules.assets.purchaseDate') }}"/>
                <x-datepicker class="block w-full mt-1" wire:model.live="purchaseDate" id="purchaseDate" autocomplete="off" placeholder="{{ __('modules.assets.purchaseDate') }}" :maxDate="true"/>
                <x-input-error for="purchaseDate" class="mt-2" />
            </div>

            <div>
                <x-label for="documentPath" value="{{ __('modules.user.uploadDocument') }}"/>
                <input class="block w-full mt-1 text-sm border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 text-slate-500" type="file" wire:model.live="documentPath">
                @if ($documentPath)
                    <div class="mt-2">
                        {{ $documentPath }}
                    </div>
