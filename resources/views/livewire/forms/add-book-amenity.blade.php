    ...
    <form wire:submit="submitBooking">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <ul class="grid w-full lg:gap-6 gap-4 grid-cols-4 md:grid-cols-6">
                    @foreach ($availableSlots as $slot)
                        <li wire:key='slot.{{ $loop->index }}.{{ microtime() }}'>
                            <input  type="checkbox" id="slot{{ $loop->index }}"  wire:model.live="bookingTime" value="{{ $slot['time'] }}" class="hidden peer"
                ...
                @if ($bookingTime && $availablePersons && count($this->bookingTime) == 1)
                    <p class="mt-4 text-sm flex items-center text-gray-700 dark:text-gray-300">
                        <x-alert type="secondary">{{ __('modules.bookAmenity.totalPersons') }}: {{ $availablePersons }}</x-alert>
                    </p>
                @endif
                <x-input-error for="bookingTime" class="mt-2" />
            </div>
        @endif
        @if ($showPersonsField && count($this->bookingTime) == 1)
            <div>
        ...
        <div class="flex w-full pb-4 space-x-4 mt-6">
            ...
            <x-button-cancel wire:click="$dispatch('hideAddBookAmenity')" wire:loading.attr="disabled">@lang('app.cancel')</x-button-cancel>
        </div>
    </form>
</div>
