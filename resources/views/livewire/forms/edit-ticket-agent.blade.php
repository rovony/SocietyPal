<div>
    <form wire:submit="submitForm">
        ...
        <div class="space-y-4">
            <div>
                <x-label for="selectedAgents" :value="__('modules.settings.societyTicketAgent')" />
                <x-select id="selectedAgents" class="block w-full mt-1" wire:model="selectedAgents" disabled>
                    <option value="">@lang('modules.settings.selectTicketAgent')</option>
                    @foreach ($users as $item)
                    <option value="{{ $item->id }}">{{ $item->name }}</option>
                    @endforeach
                </x-select>
                <x-input-error for="selectedAgents" class="mt-2" />
            </div>

            <div>
                <x-label for="ticketTypeName" :value="__('modules.settings.societyTicketType')" />

                <div x-data="{ isOpenTicket: @entangle('isOpenTicket'), selectedTicketTypes: @entangle('selectedTicketTypes') }"
                    @click.away="isOpenTicket = false">
                    <div class="relative">
                        <div @click="isOpenTicket = !isOpenTicket"
                        ...
                        <ul x-show="isOpenTicket" x-transition
                            ...
                            <li @click="$wire.toggleSelectType({ id: {{ $ticketType->id }}, type_name: '{{ addslashes($ticketType->type_name) }}' })"
                                wire:key="ticket-type-{{ $ticketType->id }}"
                        ...
                        <div class="mt-2 w-64 md:w-auto break-words">
                            ...
                            <span class="text-sm text-gray-900 dark:text-gray-400">
                                {{ implode(', ', $ticketTypes->whereIn('id',
                                $selectedTicketTypes)->pluck('type_name')->toArray()) }}
                            </span>
                        </div>
                    </div>
            ...
            <div class="flex w-full pb-4 mt-6 space-x-4">
                ...
                <x-button-cancel wire:click="$dispatch('hideEditTicketAgent')" wire:loading.attr="disabled">
                    @lang('app.cancel')</x-button-cancel>
            </div>
        </div>
    </form>
</div>
