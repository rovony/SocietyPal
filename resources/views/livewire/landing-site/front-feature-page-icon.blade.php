                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        <thead class="bg-gray-100 dark:bg-gray-700">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-xs font-medium text-right text-gray-500 uppercase dark:text-gray-400">
                                    @lang('app.action')
                                </th>
                            </tr>
                        </thead>
            ...
            <x-right-modal wire:model.live="addFeatureModal">
                ...
                <x-slot name="content">
                    <form wire:submit.prevent="saveFeature">
                        ...
                        <div class="mt-4 sm:col-span-2">
                            <label for="featureIcon" class="block mb-2 text-sm font-medium text-gray-700">
                                @lang('modules.settings.featureIcon')
                            </label>

                        <div class="relative" x-data="{ open: false }">
                            <div
                                class="flex items-center w-full px-3 py-2 bg-white border rounded cursor-pointer"
                                @click="open = !open"
                            >
                                @if ($selectedIcon)
                                    <x-dynamic-component :component="'heroicon-o-' . $selectedIcon" class="w-5 h-5 mr-2 text-gray-600" />
                            ...
                            <div
                                ...
                                class="absolute z-50 w-full mt-1 overflow-y-auto bg-white border border-gray-300 rounded shadow-lg max-h-60"
                                x-cloak
                            >
                                <div class="p-2">
                                    <input
                                        ...
                                        class="w-full px-2 py-1 mb-2 border border-gray-300 rounded"
                                    >
                                </div>
        ...
        <x-right-modal wire:model.live="showEditFrontFeatureModal">
            ...
            {{-- <x-confirmation-modal wire:model="confirmDeleteCustomerModal">
                ...
                <x-slot name="footer">
                    ...
                    @if ($frontDetail)
                    <x-danger-button class="ml-3" wire:click='deleteCustomer({{ $frontDetail->id }}, true)' wire:loading.attr="disabled">
                        @lang('modules.customer.deleteWithOrder')
                    </x-danger-button>
                    <x-danger-button class="ml-3" wire:click='deleteCustomer({{ $frontDetail->id }})' wire:loading.attr="disabled">
                        {{ __('app.delete') }}
                    </x-danger-button>
                    @endif
                </x-slot>
