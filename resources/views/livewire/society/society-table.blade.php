<div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        <thead class="bg-gray-100 dark:bg-gray-700">
                            <tr>
                                ...
                                <th scope="col"
                                    class="py-2.5 px-4 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">
                                    @lang('saas.societyName')
                                </th>
                                <th scope="col"
                                    class="py-2.5 px-4 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">
                                    @lang('saas.societyEmailAddress')
                                </th>
                                <th scope="col"
                                    class="py-2.5 px-4 text-xs font-medium text-left text-gray-500 uppercase dark:text-gray-400">
                                    @lang('saas.package')
                            ...
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700" wire:key='member-{{ $item->id . rand(1111, 9999) . microtime() }}' wire:loading.class.delay='opacity-10'>
                                ...
                                <td class="py-2.5 px-4 text-sm text-gray-900 whitespace-nowrap dark:text-white inline-flex gap-2 items-center">
                                    ...
                                    <div class="flex flex-col items-start gap-y-1">
                                        <a href="{{ route('superadmin.society.show', $item->slug) }}" wire:navigate class="underline underline-offset-1 font-medium">{{ $item->name }}</a>
                                ...
                                <td class="py-2.5 px-4 space-x-2 whitespace-nowrap text-right">
                                    <x-dropdown align="right">
                                        ...
                                        <x-slot name="content">
                                            ...
                                            <x-dropdown-link wire:click='showEditSociety({{ $item->id }})' wire:key='member-edit-{{ $item->id . microtime() }}'>
                                                @lang('app.update')
                                            </x-dropdown-link>
                                            <x-dropdown-link wire:click="showDeleteSociety({{ $item->id }})"  wire:key='member-del-{{ $item->id . microtime() }}' class="text-red-600 dark:text-red-400">
                                                @lang('app.delete')
                                            </x-dropdown-link>
    ...
    <x-right-modal wire:model.live="showEditSocietyModal">
        <x-slot name="title">
            {{ __("saas.editSociety") }}
        </x-slot>
        ...
        <x-slot name="content">
            @if ($society)
            @livewire('forms.editSociety', ['society' => $society], key(str()->random(50)))
            @endif
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$set('showEditSocietyModal', false)" wire:loading.attr="disabled">
                {{ __('app.close') }}
            </x-secondary-button>
        </x-slot>
    </x-right-modal>

    <x-confirmation-modal wire:model="confirmDeleteSocietyModal">
        <x-slot name="title">
            @lang('saas.deleteSociety')
        </x-slot>

        <x-slot name="content">
            @lang('saas.deleteSocietyMessage')
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmDeleteSocietyModal')" wire:loading.attr="disabled">
                {{ __('app.cancel') }}
            </x-secondary-button>

            @if ($society)
            <x-danger-button class="ml-3" wire:click='deleteSociety({{ $society->id }})' wire:loading.attr="disabled">
                {{ __('app.delete') }}
            </x-danger-button>
            @endif
         </x-slot>
    ...
    <x-right-modal wire:model.live="showChangePackageModal">
        <x-slot name="title">
            {{ __("modules.society.updatePackage") }}
        </x-slot>

        <x-slot name="content">
            @if ($society)
            @livewire('forms.update-package', ['society' => $society], key(str()->random(50)))
            @endif
        </x-slot>
    ...
    <x-dialog-modal wire:model.live="showRejectionReasonModal">
        <x-slot name="title">
            @lang('modules.society.rejectionReason')
        </x-slot>

        <x-slot name="content">
            <x-textarea id="rejectionReason" class="block mt-1 w-full" data-gramm="false" name="rejectionReason" placeholder="{{ __('modules.society.rejectionReasonPlaceholder') }}" rows="3" wire:model='rejectionReason' />
