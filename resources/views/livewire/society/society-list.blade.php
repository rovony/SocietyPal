    ...
    <div>
        ...
        <div class="p-4 bg-white block sm:flex items-center justify-between dark:bg-gray-800 dark:border-gray-700">
            <div class="w-full mb-1">
                <div class="mb-4">
                    <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">@lang('modules.society.societies')</h1>
                </div>
                <div class="items-center justify-between block sm:flex ">
                    <div class="flex items-center mb-4 sm:mb-0">
                        <form class="sm:pr-3" action="#" method="GET">
                            ...
                            <div class="relative w-48 mt-1 sm:w-64 xl:w-96">
                                <x-input id="menu_name" class="block mt-1 w-full" type="text" placeholder="{{ __('saas.searchByNameEmail') }}" wire:model.live.debounce.500ms="search"  />
                            </div>
                        </form>
                        ...
                        <button wire:click="$set('filterStatus', 'pending')"
                            ...
                            @lang('modules.society.needApproval')
                    ...
                    <x-button type='button' wire:click="$set('showAddSociety', true)" >@lang('modules.society.addSociety')</x-button>

                </div>
            </div>
        </div>

        <livewire:society.society-table :search='$search' :filterStatus='$filterStatus' key='society-table-{{ microtime() }}' />

    </div>



    <x-right-modal wire:model.live="showAddSociety">
        <x-slot name="title">
            {{ __("modules.society.addSociety") }}
        </x-slot>

        <x-slot name="content">
            @livewire('forms.addSociety')
        </x-slot>
    </x-right-modal>
