<div>
    <div class="h-screen" wire:key="user-management">
        ...
        <div class="p-4 bg-white block sm:flex items-center justify-between dark:bg-gray-800 dark:border-gray-700">
            <div class="w-full">
                <div class="mb-4">
                    <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">@lang('menu.userManagement')</h1>
                </div>
                <div class="items-center justify-between block sm:flex ">
                    <div class="flex items-center mb-4 sm:mb-0">
                        ...
                        <x-secondary-button wire:click="$dispatch('showUserFilters')" class="ml-1">
                    ...
                    <div class="inline-flex items-center gap-4">
                        ...
                        @if(user_can('Create User'))
                            <x-button type='button' wire:click="$set('showAddUser', true)" >@lang('app.add')</x-button>
                        @endif
                    </div>
        ...
        <livewire:user.user-table :search='$search'  key='user-table-{{ microtime() }}' />

    </div>
    <x-right-modal wire:model.live="showAddUser">
        <x-slot name="title">
            {{ __("modules.user.addUser") }}
        </x-slot>

        <x-slot name="content">
            @livewire('forms.addUser')
        </x-slot>
    </x-right-modal>
</div>
