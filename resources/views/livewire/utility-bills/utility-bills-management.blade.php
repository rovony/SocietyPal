<div class="px-4 pt-2 bg-white xl:grid-cols-3 xl:gap-4 dark:bg-gray-900">
    ...
    <div class="flex items-center justify-end">
        @if($showActions)
            <x-dropdown dropdownClasses="z-50">
                ...
                <x-slot name="content">
                    @if(user_can('Delete Utility Bills'))
                ...
                <div class="h-screen overflow-x-auto shadow md:h-auto md:overflow-x-visible">
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700" wire:key='member-list-{{ microtime() }}'>
                            @forelse ($utilityBillsData as $item)
                                ...
                                <td class="py-2.5 px-4 space-x-2 whitespace-nowrap text-right">
                                    <x-dropdown align="right">
                                        ...
                                        <x-slot name="content">
                                            ...
                                            <x-dropdown-link wire:click="showUtilityBill({{ $item->id }})" class="text-yellow-500 dark:text-yellow-400">
                                                @lang('app.view')
                                            </x-dropdown-link>

                                            @if(user_can('Update Utility Bills'))
                                                <x-dropdown-link wire:click="showEditUtilityBill({{ $item->id }})" wire:key='member-edit-{{ $item->id . microtime() }}'>
                                                    @lang('app.update')
                                                </x-dropdown-link>
                                            @endif
                                            ...
                                            @if($item->status == "paid")
                                                ...
                                                <x-dropdown-link href="{{ route('utilitybills.print', $item->id) }}" target="_blank" class="text-purple-500 dark:text-purple-400">
                                            ...
                                            @if($item->status == "unpaid")
                                                ...
                                                @if(user_can('Delete Utility Bills'))
                                                    <x-dropdown-link wire:click="showDeleteUtilityBill({{ $item->id }})" class="text-red-500 dark:text-red-400">
                                                        @lang('app.delete')
                                                    </x-dropdown-link>
                                                @endif
                            ...
                            @empty
                                <x-no-results :message="__('messages.noUtilityBillsFound')" />
                            @endforelse
    ...
    <div wire:key='utility-bill-table-paginate-{{ microtime() }}'
        class="sticky bottom-0 right-0 items-center w-full p-4 bg-white border-t border-gray-200 sm:flex sm:justify-between dark:bg-gray-800 dark:border-gray-700">
        <div class="w-full mb-4 sm:mb-0">
            {{ $utilityBillsData->links() }}
        </div>
    </div>
    <x-right-modal wire:model.live="showEditUtilityBillModal">
        <x-slot name="title">
            {{ __("modules.utilityBills.editUtilityBill") }}
        </x-slot>

        <x-slot name="content">
            @if ($utilityBill)
            @livewire('forms.editUtility-bills', ['utilityBill' => $utilityBill], key(str()->random(50)))
            @endif
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$set('showEditUtilityBillModal', false)" wire:loading.attr="disabled">
                {{ __('Close') }}
            </x-secondary-button>
        </x-slot>
    </x-right-modal>
    ...
    <x-right-modal wire:model.live="showUtilityBillModal">
        ...
        <x-slot name="content">
            @if ($utilityBill)
            @livewire('forms.showUtility-bills', ['utilityBill' => $utilityBill], key(str()->random(50)))
            @endif
        </x-slot>
    ...
    <x-confirmation-modal wire:model="confirmDeleteUtilityBillModal">
        <x-slot name="title">
            @lang('modules.utilityBills.deleteUtilityBill')
        </x-slot>
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmDeleteUtilityBillModal')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>

            @if ($utilityBill)
            <x-danger-button class="ml-3" wire:click='deleteUtilityBill({{ $utilityBill->id }})' wire:loading.attr="disabled">
                {{ __('Delete') }}
            </x-danger-button>
            @endif
         </x-slot>
    ...
    <x-dialog-modal wire:model.live="showPayModal">
        ...
        <x-slot name="content">
            @livewire('forms.utility-pay',['utilityBill' => $utilityBill], key(str()->random(50)))
        </x-slot>
    ...
    <x-confirmation-modal wire:model="confirmSelectedDeleteUtilityBillModal">
        <x-slot name="title">
            @lang('modules.utilityBills.deleteUtilityBill')
        </x-slot>
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmSelectedDeleteUtilityBillModal')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>
