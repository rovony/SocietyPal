<div class="px-4 pt-2 bg-white xl:grid-cols-3 xl:gap-2 sm:gap-4 dark:bg-gray-900">




    @if ($showFilters)
        @include('apartments.filters')
    @endif
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                        <thead class="bg-gray-100 dark:bg-gray-700">
                            <tr>
                                ...
                                @if(user_can('Update Apartment') || user_can('Delete User'))
                                    <th scope="col"
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700" wire:key='member-list-{{ microtime() }}'>
                            ...
                            @forelse ($apartmentManagmentsData as $item)
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700" wire:key='member-{{ $item->id . rand(1111, 9999) . microtime() }}' wire:loading.class.delay='opacity-10'>
                                ...
                                @if(user_can('Update Apartment') || user_can('Delete User'))
                                    <td class="p-4 space-x-2 text-right whitespace-nowrap">
                                        ...
                                        @if(user_can('Update Apartment'))
                                            <x-secondary-button wire:click='showEditApartmentManagement({{ $item->id }})' wire:key='member-edit-{{ $item->id . microtime() }}'
                                        ...
                                        @endif
                                        @if(user_can('Delete User'))
                                            <x-danger-button  wire:click="showDeleteApartmentManagement({{ $item->id }})"  wire:key='member-del-{{ $item->id . microtime() }}'>
    ...
    <div wire:key='apartment-table-paginate-{{ microtime() }}'
        class="sticky bottom-0 right-0 items-center w-full p-4 bg-white border-t border-gray-200 sm:flex sm:justify-between dark:bg-gray-800 dark:border-gray-700">
        <div class="mb-4 sm:mb-0 w-full">
            {{ $apartmentManagmentsData->links() }}
        </div>
    </div>

    <x-right-modal wire:model.live="showEditApartmentManagementModal">
        <x-slot name="title">
            {{ __("modules.settings.editSocietyApartmentManagement") }}
        </x-slot>

        <x-slot name="content">
            @if ($apartmentManagment)
            @livewire('forms.editApartmentManagement', ['apartmentManagment' => $apartmentManagment], key(str()->random(50)))
            @endif
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="$set('showEditApartmentManagementModal', false)" wire:loading.attr="disabled">
                {{ __('Close') }}
            </x-secondary-button>
        </x-slot>
    </x-right-modal>

    <x-confirmation-modal wire:model="confirmDeleteApartmentManagementModal">
        <x-slot name="title">
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmDeleteApartmentManagementModal')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>
            @if ($apartmentManagment)
            <x-danger-button class="ml-3" wire:click='deleteApartmentManagement({{ $apartmentManagment->id }})' wire:loading.attr="disabled">
                {{ __('Delete') }}
            </x-danger-button>
            @endif
         </x-slot>
    ...
    <x-confirmation-modal wire:model="confirmSelectedDeleteApartmentManagementModal">
        <x-slot name="title">
            @lang('modules.settings.deleteApartmentManagement')
        </x-slot>
        ...
        <x-slot name="footer">
            <x-secondary-button wire:click="$toggle('confirmSelectedDeleteApartmentManagementModal')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-secondary-button>
