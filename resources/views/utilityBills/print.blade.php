...
<head>
    ...
    <title>Utility Bill - {{ $utilityBill->apartmentId }}</title>
    <style>
        * {
...
<body class="content-wrapper">
            ...
            <tr>
                <td style="vertical-align: top;">
                    <p class="mb-0 text-black line-height f-11">
                        Utility Bill for Apartment: {{ $utilityBill->apartment_id }}
                    </p>
                </td>
                <td align="right" class="text-black f-21 text-uppercase">Utility Bill<br>
                    <table class="mt-1 text-black f-11 b-collapse rightaligned">
                        <tr>
                            ...
                            <td class="heading-table-right">
                                {{ \Carbon\Carbon::parse($utilityBill->bill_due_date)->format('d M, Y') }}
                            </td>
                        </tr>
                        <tr>
                            <td class="heading-table-left text-capitalize">Bill Date</td>
                            <td class="heading-table-right">
                                {{ \Carbon\Carbon::parse($utilityBill->bill_date)->format('d M, Y') }}
                            </td>
                        </tr>
                    </table>
                </td>
    ...
    <table width="100%" class="f-14 b-collapse">
        ...
        <tr class="text-black f-12 main-table-items">
            <td width="40%" class="border-bottom-0">
                <div>Apartment Number: {{ $utilityBill->apartment_id}}</div>
                <div>Bill Type: {{ $utilityBill->billType->name}}</div>
            </td>
            <td align="right" width="10%" class="border-bottom-0">{{ currency_format($utilityBill->bill_amount) }}</td>
            <td align="right" width="10%" class="border-bottom-0">{{ currency_format($utilityBill->bill_amount) }}</td>
            <td align="right" class="border-bottom-0">{{ currency_format($utilityBill->bill_amount) }}</td>
        </tr>
        <tr>
            ...
            <td class="total-box" align="right">
                <table width="100%" class="b-collapse">
                    <tr align="right" class="text-grey">
                        <td class="total-amt f-15">{{ currency_format($utilityBill->bill_amount) }} ({{ $currencySymbol }})</td>
                    </tr>
                </table>
            </td>
        </tr>
    ...
    <div class="mt-1 f-12">
        Date & Time: {{ $utilityBill->created_at->format('Y-m-d H:i:s') }}
    </div>
