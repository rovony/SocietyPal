            ...
            <x-slot name="trigger">
                <span class="inline-flex rounded-md">
                    <button type="button"
                        ...
                        @lang('modules.notice.filterRoles')
                        <div class="inline-flex items-center justify-center w-5 h-5 text-xs font-medium text-white bg-red-500  rounded-md  dark:border-gray-900 ml-1 {{ count($filterRoles) == 0 ? 'hidden' : '' }}">{{ count($filterRoles) }}</div>
            ...
            <x-slot name="content">
                ...
                @foreach ($roles as $role)
                    <x-dropdown-link wire:key='role-option-{{ $role->id }}' onclick="event.stopPropagation()">
                        <input id="role-{{ $role->id }}" type="checkbox" value="{{ $role->id }}" wire:model.live='filterRoles'
