...
<head>
    ...
    @include('sections.theme_style', ['baseColor' => $globalSetting->theme_rgb, 'baseColorHex' => $globalSetting->theme_hex,])
    ...
    <script>
        (function () {
                ...
                if (!sessionStorage.getItem('pwaDismissed')) {
                    const handleInstall = () => {
                        if (deferredPrompt) {
                            ...
                            // Clean up listeners
                            window.removeEventListener('scroll', handleInstall);
                            window.removeEventListener('click', handleInstall);
                        }
                    };

                    // Show on first interaction
                    window.addEventListener('scroll', handleInstall, { once: true });
                    window.addEventListener('click', handleInstall, { once: true });
                }
            });

            // ✅ iOS Install Instructions (Safari only)
            if (isIOS && !isInStandaloneMode) {
                ...
                window.addEventListener('click', showIOSInstallInstructions, { once: true });
            }
        })();
    </script>
@endif
