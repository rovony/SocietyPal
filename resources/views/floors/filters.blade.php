        ...
        <x-dropdown align="left" dropdownClasses="z-10">
            ...
            <x-slot name="content">
                ...
                @foreach ($tower as $item)
                    <x-dropdown-link wire:key="tower-option-{{ $item->id }}" onclick="event.stopPropagation()">
                        <input id="tower-{{ $item->id }}" type="checkbox"
                            value="{{ $item->tower_name }}" wire:model.live="filterTower"
