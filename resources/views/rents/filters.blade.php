        ...
        <x-dropdown align="left" dropdownClasses="z-10">
            <x-slot name="trigger">
                <span class="inline-flex rounded-md">
                    <button type="button"
                        ...
                        @lang('modules.maintenance.filterYears')
                        <div class="inline-flex items-center justify-center w-5 h-5 text-xs font-medium text-white bg-red-500  rounded-md  dark:border-gray-900 ml-1 {{ count($filterYears) == 0 ? 'hidden' : '' }}">{{ count($filterYears) }}</div>
            ...
            <x-slot name="content">
                ...
                @for ($year = now()->year; $year >= now()->year - 5; $year--)
                    <x-dropdown-link wire:key='year-option-{{ $year }}' onclick="event.stopPropagation()">
                        <input id="year-{{ $year }}" type="checkbox" value="{{ $year }}" wire:model.live='filterYears'
                        ...
                        <label for="year-{{ $year }}" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">
                            {{ $year }}
                        </label>
                    </x-dropdown-link>
                @endfor
        ...
        <x-dropdown align="left" dropdownClasses="z-10">
            <x-slot name="trigger">
                <span class="inline-flex rounded-md">
                    <button type="button"
                        class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                        @lang('modules.maintenance.filterMonths')
                        <div class="inline-flex items-center justify-center w-5 h-5 text-xs font-medium text-white bg-red-500  rounded-md  dark:border-gray-900 ml-1 {{ count($filterMonths) == 0 ? 'hidden' : '' }}">{{ count($filterMonths) }}</div>
                        <svg class="-mr-1 ml-1.5 w-5 h-5" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                        <path clip-rule="evenodd" fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" />
                        </svg>
                    </button>
            ...
            <x-slot name="content">
                ...
                @foreach ($this->months as $key => $label)
                    <x-dropdown-link class="flex items-center" onclick="event.stopPropagation()">
                        <input id="month-{{ $key }}" type="checkbox" value="{{ $key }}" wire:model.live='filterMonths'
                        class="w-4 h-4 bg-gray-100 border-gray-300 rounded text-gray-600 focus:ring-gray-500 dark:focus:ring-gray-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                        <label for="month-{{ $key }}" class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">
                            {{ $label }}
                        </label>
                    </x-dropdown-link>
                @endforeach
    ...
    <div>
        <x-dropdown align="left" dropdownClasses="z-10">
            ...
            <x-slot name="content">
                ...
                @foreach (['paid' => __('modules.rent.paid'), 'unpaid' => __('modules.rent.unpaid')] as $key => $label)
