            ...
            <x-slot name="trigger">
                <span class="inline-flex rounded-md">
                    <button type="button"
                        ...
                        @lang('modules.settings.filterApartmentType')
                        <div
                            class="inline-flex items-center justify-center w-5 h-5 text-xs font-medium text-white bg-red-500 rounded-md dark:border-gray-900 ml-1 {{ count($filterApartmentsType) == 0 ? 'hidden' : '' }}">
                            {{ count($filterApartmentsType) }}
                        </div>
                ...
                @foreach ($apartments as $apartment)
                    <x-dropdown-link wire:key="apartment-option-{{ $apartment->id }}" onclick="event.stopPropagation()">
                        <input id="apartment-{{ $apartment->id }}" type="checkbox"
                            value="{{ $apartment->apartment_type }}" wire:model.live="filterApartmentsType"
        ...
        <x-dropdown align="left" dropdownClasses="z-10">
            <x-slot name="trigger">
                <span class="inline-flex rounded-md">
                    <button type="button"
                        class="inline-flex items-center px-3 py-2 text-sm font-medium leading-4 text-gray-500 transition duration-150 ease-in-out border border-transparent rounded-md hover:text-gray-700 focus:outline-none">
                        @lang('modules.settings.filterFloor')
                        <div
                            class="inline-flex items-center justify-center w-5 h-5 text-xs font-medium text-white bg-red-500 rounded-md dark:border-gray-900 ml-1 {{ count($filterFloor) == 0 ? 'hidden' : '' }}">
                            {{ count($filterFloor) }}
                        </div>
                        <svg class="-mr-1 ml-1.5 w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                            xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            ...
            <x-slot name="content">
                ...
                @foreach ($floor as $floors)
                    <x-dropdown-link wire:key="floors-option-{{ $floors->id }}" onclick="event.stopPropagation()">
                        <input id="floors-{{ $floors->id }}" type="checkbox"
                            value="{{ $floors->floor_name }}" wire:model.live="filterFloor"
                            class="w-4 h-4 text-gray-600 bg-gray-100 border-gray-300 rounded focus:ring-gray-500 dark:focus:ring-gray-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                        <label for="floors-{{ $floors->id }}"
                            class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">
                            {{ $floors->floor_name }}
                        </label>
                    </x-dropdown-link>
    ...
    <div>
        <x-dropdown align="left" dropdownClasses="z-10">
            ...
            <x-slot name="content">
                ...
                @foreach (['not_sold' => __('modules.settings.notSold'), 'occupied' => __('modules.settings.occupied'),'available_for_rent' => __('modules.settings.availableForRent'),'rented' => __('modules.settings.rented')] as $key => $label)
