...
<body class="content-wrapper">
        ...
        <tbody>
            ...
            <tr>
                <td style="vertical-align: top;">
                    <img src="{{ asset(global_setting()->logoUrl) }}" alt="{{ global_setting()->name }}" /> 
    ...
    <table width="100%" class="f-14 b-collapse">
        ...
        <tr class="f-12 main-table-items text-black">
            ...
            <td align="right" class="border-bottom-0">{{ $invoice->total ?? 0 }}</td>
            <td align="right" class="border-bottom-0">{{ $invoice->total ?? 0 }}</td>
        </tr>
        <!-- Table Row End -->
    ...
    <div class="f-12 mt-1">
        @if($invoice->gateway_name != 'offline')
            @lang('modules.billing.paidVia') {{ $invoice->gateway_name }} - {{ $invoice->created_at->format('Y-m-d H:i:s') }}
        @else
            @lang('modules.billing.paid') {{ $invoice->gateway_name }} - {{ $invoice->created_at->format('Y-m-d H:i:s') }}
        @endif
    </div>
