      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center justify-start">
          ...
          <a href="{{ route('dashboard') }}" class="flex ltr:ml-2 rtl:mr-2 items-center">
            <img src="{{ society()->logo_url }}" class="h-8 ltr:mr-3 rtl:ml-3" alt="App Logo" />
            @if (society()->show_logo_text)
            <span class="self-center text-xl font-semibold sm:text-xl whitespace-nowrap dark:text-white hidden lg:block ltr:mr-2 rtl:ml-2">{{ Str::limit(society()->name, 10) }}</span>
            @endif
          </a>
        ...
        <div class="flex items-center gap-4 w-fit justify-end">
          ...
          @if (session()->has('impersonate_society_id'))
              @livewire('society.stop-impersonate-society')
          @elseif (session()->has('impersonate_user_id'))
              @livewire('society.stop-impersonate')
          @endif

          @if (society()->package->package_type == \App\Enums\PackageType::DEFAULT)
          ...
          @elseif (society()->package->package_type == \App\Enums\PackageType::TRIAL)
          <a href="{{ route('pricing.plan') }}" wire:navigate class="hidden lg:inline-flex" >
              <x-secondary-button>
                  @php
                      $daysLeftInTrial = floor(now(timezone())->diffInDays(\Carbon\Carbon::parse(society()->trial_ends_at)->addDays(1)));
                  @endphp
