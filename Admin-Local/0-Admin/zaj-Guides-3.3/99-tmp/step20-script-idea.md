...
**Add to Step 20** - right before the commit section. It makes perfect sense as the **final verification** before committing everything.

---

## **🔧 The Script**

Add this to **Step 20** right after the "Pre-Commit Verification" section:

### **Add to Step 20: Complete Phase 2 Verification**

```bash
# Create comprehensive Phase 2 verification script
echo "🔧 Creating Phase 2 completion verification script..."

cat > scripts/verify_phase2_complete.sh << 'EOF'
#!/bin/bash
...
if [ -f "scripts/link_persistent_dirs.sh" ]; then
    ...
    if [ -x "scripts/link_persistent_dirs.sh" ]; then
        ...
        check_fail "link_persistent_dirs.sh not executable - run 'chmod +x scripts/link_persistent_dirs.sh'"
    fi
else
    check_fail "scripts/link_persistent_dirs.sh missing"
fi

# Check verification script
if [ -f "scripts/verify_persistence.sh" ]; then
    check_pass "verify_persistence.sh exists"
    if [ -x "scripts/verify_persistence.sh" ]; then
        check_pass "verify_persistence.sh is executable"
    else
        check_warn "verify_persistence.sh not executable"
    fi
else
    check_warn "scripts/verify_persistence.sh missing (optional)"
fi
...
- ✅ **Every step verified** - Dependencies through documentation
- ✅ **Security checks** - No .env committed, proper .gitignore
- ✅ **Executable permissions** - Scripts can actually run
- ✅ **Git status** - Repository ready for commit

### **Clear Output**
- ✅ **Color-coded** results (green/red/yellow)
- ✅ **Specific fixes** suggested for each error
- ✅ **Summary report** with clear next steps

### **Smart Exit Codes**
- ✅ **Exit 0** - Ready for deployment
- ✅ **Exit 1** - Must fix errors first
- ✅ **Warnings allowed** - Can proceed with minor issues

**This gives you the "one command confidence check" before Phase 3!**