...
# Below is current build Pipeline we have.


## 1 Create Laravel Directorie
- Custom 
- Version: NA
- Stop the deployment if the command fails? `YES`
```
#!/bin/bash
set -e
...
- Composer  
- Version: 2 (unless project neeeds dif), acceptable versions: xxx,xxx..
- Stop the deployment if the command fails? `YES`
```
#!/bin/bash
set -e
# Note: Composer (composer install) Uses composer.lock by default 
...
- NPM  
- Version: xx (unless project neeeds dif), acceptable versions: xxx,xxx..
- Stop the deployment if the command fails? `YES`
```
#!/bin/bash
set -e
