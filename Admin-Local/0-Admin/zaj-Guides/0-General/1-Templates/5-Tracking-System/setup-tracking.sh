...
create_directory() {
    ...
    if [[ ! -d "$dir" ]]; then
        mkdir -p "$dir"
        log_success "Created: $(realpath --relative-to="$PROJECT_ROOT" "$dir")"
    else
        log_info "Exists: $(realpath --relative-to="$PROJECT_ROOT" "$dir")"
    fi
}
...
EOF
        log_success "Created README: $(realpath --relative-to="$PROJECT_ROOT" "$dir/README.md")"
    fi
}
