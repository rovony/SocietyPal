...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Global navigation function for HTML button compatibility
...
function setupSavePathListener() {
    ...
    if (savePathField) {
        savePathField.addEventListener("input", function () {
            ...
            // Update storage manager backup path
            if (window.storageManager) {
...
function startAutoSave() {
    ...
    setInterval(() => {
        if (window.wizard && window.storageManager) {
            window.wizard.collectCurrentStepData();
            window.wizard.saveConfiguration();
        }
    }, 30000);
